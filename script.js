function switchTab(buttonId, contentId) {
  ...
  // Check if both buttonId and contentId exist
  if (!buttonIds.includes(buttonId)) {
    console.error(`Button ID "${buttonId}" is invalid.`);
    return;
  }

  if (!contentIds.includes(contentId)) {
    console.error(`Content ID "${contentId}" is invalid.`);
    return;
  }
...
buttonIds.forEach((buttonId, index) => {
  ...
  if (button) {
    button.addEventListener("click", () => switchTab(buttonId, contentId));
  } else {
